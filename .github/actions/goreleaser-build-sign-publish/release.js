...
function main() {
  ...
  const results = dockerImages.map((image) => {
    try {
      console.log(
        `Checking version for image: ${image}, expected version: ${pluginVersion}, expected SHA: ${repoSha}`
      );
      ...
      if (version.trim() !== pluginVersion) {
        throw new Error(
          `Version mismatch: Expected ${pluginVersion}, got ${version.trim()}`
        );
      }
...
function constructGoreleaserCommand(releaseType, version, goreleaserConfig) {
  ...
  if (releaseType === "merge") {
    return `PLUGIN_VERSION=${version} goreleaser ${subCmd} ${flagsStr}`;
  } else {
    return `PLUGIN_VERSION=${version} goreleaser ${subCmd} --config ${goreleaserConfig} ${flagsStr}`;
  }
}
...
function getVersion() {
  try {
    ...
    console.log("Looking for plugin version in package.json at: ", pkgPath);
