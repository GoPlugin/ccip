...
contract EVM2EVMOnRamp is IEVM2AnyOnRamp, ILinkAvailable, AggregateRateLimiter, ITypeAndVersion {
  ...
  /// @dev The pli token address - known to pay nops for their work
  ...
  function forwardFromRouter(
    ...
    // Convert feeToken to pli if not already in link
    if (message.feeToken == i_linkToken) {
      // Since there is only 1b pli this is safe
      s_nopFeesJuels += uint96(feeTokenAmount);
    } else {
      // the cast from uint256 to uint96 is considered safe, uint96 can store more than max supply of pli token
  ...
  function _setNops(NopAndWeight[] memory nopsAndWeights) internal {
    ...
    for (uint256 i = 0; i < numberOfNops; ++i) {
      // Make sure the PLI token is not a nop because the pli token doesn't allow
  ...
  /// The NOP pli balance is not withdrawable.
  ...
  function withdrawNonLinkFees(address feeToken, address to) external {
    ...
    // We require the pli balance to be settled before allowing withdrawal of non-link fees.
    int256 pliAfterNopFees = linkAvailableForPayment();
    if (linkAfterNopFees < 0) revert LinkBalanceNotSettled();

    if (feeToken == i_linkToken) {
      // Withdraw only the left over pli balance
