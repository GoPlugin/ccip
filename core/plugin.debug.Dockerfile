...
RUN if [ "$GO_COVER_FLAG" = "true" ]; then \
        ...
        make install-plugin-delve; \
    fi

# Pli LOOP Plugin source dirs with simple names
...
# Install dlv
ENV GOPATH=/home/${PLUGIN_USER}/go
...
CMD ["dlv", "exec", "/usr/local/bin/plugin", "--accept-multiclient", "--headless", "--listen=0.0.0.0:40000", "--api-version=2", "--", "local", "node"]
