...
func main() {
	...
	subscriptionBalanceJuelsString := flag.String("subscription-balance", constants.SubscriptionBalanceJuels, "amount to fund subscription with Pli token (Juels)")
	...
	nativeOnly := flag.Bool("native-only", false, "if true, pli and pli feed are not set up. Only used in v2 plus")
	linkAddress := flag.String("link-address", "", "address of pli token")
	linkEthAddress := flag.String("link-eth-feed", "", "address of pli eth feed")
