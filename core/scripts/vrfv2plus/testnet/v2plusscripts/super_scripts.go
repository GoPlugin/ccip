...
func SmokeTestVRF(e helpers.Environment) {
	...
	linkAddress := smokeCmd.String("link-address", "", "address of pli token")
	linkNativeAddress := smokeCmd.String("link-native-feed", "", "address of pli native feed")
...
func DeployUniverseViaCLI(e helpers.Environment) {
	...
	// required flags
	nativeOnly := deployCmd.Bool("native-only", false, "if true, pli and pli feed are not set up")
	linkAddress := deployCmd.String("link-address", "", "address of pli token")
	linkNativeAddress := deployCmd.String("link-native-feed", "", "address of pli native feed")
	...
	subscriptionBalanceJuelsString := deployCmd.String("subscription-balance", "1e19", "amount to fund subscription with Pli token (Juels)")
	...
	if *nativeOnly {
		if *linkAddress != "" || *linkNativeAddress != "" {
			panic("native-only flag is set, but pli address or pli native address is provided")
		}
		if *subscriptionBalanceJuelsString != "0" {
			panic("native-only flag is set, but pli subscription balance is provided")
		}
	}
...
func VRFV2PlusDeployUniverse(e helpers.Environment,
	...
	if subscriptionBalanceJuels.Cmp(big.NewInt(0)) > 0 {
		fmt.Println("\nFunding subscription with Pli Token.", subscriptionBalanceJuels, "juels...")
		EoaFundSubWithLink(e, *coordinator, contractAddresses.LinkAddress, subscriptionBalanceJuels, subID)
	} else {
		fmt.Println("Subscription", subID, "NOT getting funded with Pli Token. You must fund the subscription in order to use it!")
	}
...
func DeployWrapperUniverse(e helpers.Environment) {
	...
	linkAddress := cmd.String("link-address", "", "address of pli token")
	...
	coordinatorGasOverheadLink := cmd.Uint("coordinator-gas-overhead-link", 74_000, "amount of gas overhead in coordinator fulfillment for pli payment")
	...
	wrapperLinkPremiumPercentage := cmd.Uint("wrapper-link-premium-percentage", 25, "gas premium charged by wrapper for pli payment")
	...
	fulfillmentFlatFeeLinkDiscountPPM := cmd.Uint("fulfillment-flat-fee-link-discount-ppm", 500, "the pli flat fee discount in ppm to charge for fulfillment denominated in native")
	// only necessary for Optimism coordinator contract
