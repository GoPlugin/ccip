...
// Note we have to consider the pending reqs already in the txm as already "spent" pli or native,
// using a max pli or max native consumed in their metadata.
...
func (lsn *listenerV2) processRequestsPerSubBatch(
	...
	// process the native and pli requests in parallel
	...
	wg.Wait()
	// combine the processed pli and native requests into the processed map
...
func (lsn *listenerV2) enqueueForceFulfillment(
	...
	return lsn.chain.TxManager().CreateTransaction(ctx, txmgr.TxRequest{
		...
		Meta: &txmgr.TxMeta{
			...
			// No max pli since simulation failed
		},
	})
}
...
func (lsn *listenerV2) processRequestsPerSub(
	...
	for _, req := range reqs {
		if req.req.NativePayment() {
			...
			if !lsn.inflightCache.Contains(req.req.Raw()) {
				...
				lsn.l.Debugw("Skipping pli request because it is already inflight",
					"reqID", req.req.RequestID())
			}
		}
	}
	...
	wg.Wait()
	// combine the native and pli processed requests into the processed map
	for k, v := range nativeProcessed {
		processed[k] = v
	}
	for k, v := range linkProcessed {
...
// then simulate the transaction at the max gas price to determine its maximum pli cost.
