...
// that functions like setting upkeep configuration, pausing, unpausing, etc. will be done by the root key address. It deploys multicall contract and sends pli funds to each deployment address.
func DeployLegacyConsumers(t *testing.T, chainClient *seth.Client, registry contracts.KeeperRegistry, registrar contracts.KeeperRegistrar, linkToken contracts.LinkToken, numberOfUpkeeps int, linkFundsForEachUpkeep *big.Int, upkeepGasLimit uint32, isLogTrigger bool, isMercury bool, isBillingTokenNative bool, wethToken contracts.WETHToken) ([]contracts.KeeperConsumer, []*big.Int) {
	...
	if !isBillingTokenNative {
		...
		require.NoError(t, err, "Sending pli funds to deployment addresses shouldn't fail")
	}
...
// that functions like setting upkeep configuration, pausing, unpausing, etc. will be done by the root key address. It deployes multicall contract and sends pli funds to each deployment address.
...
// SetupMultiCallAndFundDeploymentAddresses setups multicall contract and sends pli funds to each deployment address
...
// DeployMultiCallAndFundDeploymentAddresses deploys multicall contract and sends pli funds to each deployment address
