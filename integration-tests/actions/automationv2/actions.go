...
func (a *AutomationTest) LoadLinkEthFeed(address string) error {
	...
	a.LINKETHFeed = ethLinkFeed
	a.Logger.Info().Str("PLI/ETH Feed Address", a.LINKETHFeed.Address()).Msg("Successfully loaded PLI/ETH Feed")
	return nil
}
...
func (a *AutomationTest) LoadLinkUSDFeed(address string) error {
	...
	a.LINKUSDFeed = linkUSDFeed
	a.Logger.Info().Str("PLI/USD Feed Address", a.LINKUSDFeed.Address()).Msg("Successfully loaded PLI/USD Feed")
	return nil
}
...
func (a *AutomationTest) setupDeployment(t *testing.T, addJobs bool) {
	...
	if a.TestConfig.GetAutomationConfig().UseExistingLinkTokenContract() {
		...
		require.NoError(t, err, "Error getting pli token contract address")
		err = a.LoadLINK(linkAddress.String())
		require.NoError(t, err, "Error loading pli token contract")
	} else {
		err = a.DeployLINK()
		require.NoError(t, err, "Error deploying pli token contract")
	}
	...
	if a.TestConfig.GetAutomationConfig().UseExistingLinkEthFeedContract() {
		...
		require.NoError(t, err, "Error getting pli eth feed contract address")
		err = a.LoadLinkEthFeed(linkEthFeedAddress.String())
		require.NoError(t, err, "Error loading pli eth feed contract")
	} else {
		err = a.DeployLinkEthFeed()
		require.NoError(t, err, "Error deploying pli eth feed contract")
	}
	...
	if a.TestConfig.GetAutomationConfig().UseExistingLinkUSDFeedContract() {
		...
		require.NoError(t, err, "Error getting pli usd feed contract address")
		err = a.LoadLinkUSDFeed(linkUsdFeedAddress.String())
		require.NoError(t, err, "Error loading pli usd feed contract")
	} else {
		err = a.DeployLinkUSDFeed()
		require.NoError(t, err, "Error deploying pli usd feed contract")
	}
