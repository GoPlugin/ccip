...
- If there are sensitive details like private keys, credentials in test config, they can be specified in a separate dotenv file as env vars
  - The `~/.testsecrets` file in home directory is automatically loaded and should have all test secrets as env vars. Learn more about it [here](https://github.com/goplugin/plugin-testing-framework/blob/main/config/README.md#test-secrets) 
...
Test needs network/chain details to be set through configuration. Set network urls in ~/.testsecrets [see docs](https://github.com/goplugin/plugin-testing-framework/blob/main/config/README.md#test-secrets).
